...
The numbers of free generators in a minimal free resolution are called **Betti numbers**. The total Betti number $\beta_i$ records only how many generators occur in homological degree $i$. A **graded Betti number** $\beta_{i,j}$ also remembers the ordinary polynomial degree $j$. For our example, $\beta_{1,2}=2$ means that there are two free generators in homological degree 1 and polynomial degree 2, corresponding to $x_1x_3$ and $x_2x_4$. Likewise, $\beta_{2,4}=1$ means that there is one free generator in homological degree 2 and polynomial degree 4, corresponding to the relation between those two generators.
