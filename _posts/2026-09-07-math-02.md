...
If we forget which variables occur and remember only how many occur, then we recover the graded Betti numbers: $\beta_{i,j}=\sum_{\substack{W\subseteq[n]\\|W|=j}} \beta_{i,W}.$
...
There is a special kind of ideal associated to $\Delta$, which is called a **Stanley--Reisner Ideal** $I_\Delta$. A squarefree monomial $x_{i_1}\cdots x_{i_r}$ belongs to $I_\Delta$ when $\{i_1,\dots,i_r\}$ is not a face of $\Delta$. The minimal nonfaces give the minimal monomial generators. 
